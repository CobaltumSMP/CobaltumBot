...
public class HelpCommand implements Command {
    ...
    public void execute(Message message, List<String> args) {
        ...
        int length = Main.COMMANDS.keySet().stream().map(String::length).max(Integer::compareTo).orElse(-1);

        List<String> helpList = new ArrayList<>();

        // Show only the commands the author has access to
        User author = message.getUserAuthor().get();
        Main.COMMANDS.values().stream().filter(command -> Roles.checkRoles(command.roleRequired(), author)).forEach(command -> {
        ...
        EmbedBuilder embed = new EmbedBuilder().setColor(Color.decode("#085a7a"))
                ...
                .setAuthor(message.getApi().getYourself().getDiscriminatedName(), "", message.getApi().getYourself().getAvatar())
